...
export default function MessagesScreen() {
  ...
  useEffect(() => {
    ...
    searchTimerRef.current = setTimeout(fetchContacts, 500);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessageText('');
        // Only reload all threads when starting a brand-new conversation
            ...
            return (
              <View style={[styles.bubble, isMe ? styles.bubbleMine : styles.bubbleTheirs]}>
                ...
                <Text style={[styles.bubbleTime, isMe && { color: 'rgba(255,255,255,0.6)' }]}>
                  {isNaN(new Date(item.createdAt).getTime()) ? '' : new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </Text>
              </View>
            );
          }}
            ...
            return (
              <TouchableOpacity style={styles.threadItem} onPress={() => setCurrentThread(item)}>
                ...
                <Text style={styles.threadTime}>
                  {lastMsg && !isNaN(new Date(lastMsg.createdAt).getTime()) ? new Date(lastMsg.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : ''}
                </Text>
              </TouchableOpacity>
            );
          }}
