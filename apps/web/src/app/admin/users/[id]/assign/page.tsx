...
export default function AssignJobPage() {
    ...
    const [brandAllocation, setBrandAllocation] = useState<"KRUTO" | "MULUK" | "BOTH" | "">("");
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (brandAllocation) {
                payload.brandAllocation = brandAllocation;
            }
                        ...
                        <div style={{ padding: "1.5rem 2rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", textAlign: "left" }}>
                                <label style={{ fontSize: "0.875rem", fontWeight: 600, color: "#374151" }}>
                                    Bonus ($) <span style={{ fontWeight: 400, color: "#9ca3af" }}>— optional</span>
                                </label>
                                <input
                                    type="number"
                                    step="0.01"
                                ...
                                />
                                {shiftType === "recurring" && bonusAmount.trim() !== "" && (
                                    <p style={{ fontSize: "0.75rem", color: "#b45309", margin: 0 }}>
                                        ⚠️ This bonus applies to every shift generated by this recurring pattern this cycle, not just once.
                                    </p>
                                )}
                            </div>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", textAlign: "left" }}>
                                <label style={{ fontSize: "0.875rem", fontWeight: 600, color: "#374151" }}>
                                    Brand Allocation <span style={{ fontWeight: 400, color: "#9ca3af" }}>— optional</span>
                                </label>
                                <div style={{ display: "flex", gap: "0.5rem" }}>
                                    {(["KRUTO", "MULUK", "BOTH"] as const).map((opt) => {
                                        const isSelected = brandAllocation === opt;
                                        const label = opt === "KRUTO" ? "Kruto" : opt === "MULUK" ? "Muluk" : "Both";
                                        return (
                                            <button
                                                key={opt}
                                                type="button"
                                                onClick={() => setBrandAllocation(isSelected ? "" : opt)}
                                                style={{
                                                    flex: 1,
                                                    padding: "0.6rem",
                                                    borderRadius: "10px",
