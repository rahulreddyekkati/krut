...
export async function sendPasswordResetOTPEmail(email: string, otp: string): Promise<boolean> {
    ...
    if (!transporter) {
        console.warn("⚠️ SMTP not configured. Skipping password reset OTP email.");
        return false;
    }
    try {
        ...
        const htmlBody = `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 32px 24px; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px;">
                ...
                <p style="color: #0f172a; font-size: 18px; font-weight: 600; margin-top: 0; margin-bottom: 16px;">Password Reset Code</p>
                ...
                <div style="text-align: center; margin-bottom: 28px;">
                    <span style="display: inline-block; background-color: #f0fdf4; border: 1px solid #22c55e; border-radius: 8px; padding: 16px 32px; font-size: 32px; font-weight: 800; letter-spacing: 0.2em; color: #166534;">${otp}</span>
                </div>
                <p style="color: #64748b; font-size: 13px; line-height: 20px; margin-top: 0; margin-bottom: 24px;">
...
export async function sendSampleRequestEmail(
    ...
    const from = process.env.SMTP_FROM || '"Kruto Tastes" <noreply@krutotastes.com>';
    if (!transporter) {
        console.warn("⚠️ SMTP not configured. Skipping sample request email.");
        return false;
    }
    ...
    try {
        ...
        const htmlBody = `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 32px 24px; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px;">
                ...
                <p style="color: #0f172a; font-size: 18px; font-weight: 600; margin-top: 0; margin-bottom: 16px;">New Sample Request</p>
        ...
        // Single send to the whole admin group together — not a loop of per-admin sends.
        await transporter.sendMail({ from, to: recipientEmails.join(", "), subject, text: textBody, html: htmlBody });
        return true;
    } catch (error) {
        console.error(`❌ Failed to send sample request email to admins:`, error);
        return false;
    }
}
