import fs from "fs";

function normalizeWorkerName(ftmName) {
    if (!ftmName) return null;
    ftmName = ftmName.trim().replace(/^"|"$/g, '');
    const parts = ftmName.split(',');
    if (parts.length === 2) {
        return `${parts[1].trim()} ${parts[0].trim()}`;
    }
    return ftmName;
}

function parseCsvJune(content) {
    const lines = content.split('\n');
    const result = [];
    for (let i = 10; i < lines.length; i++) {
        const line = lines[i].trim();
        if (!line) continue;
        const row = [];
        let inQuotes = false;
        let currentField = '';
        for (let j = 0; j < line.length; j++) {
            const char = line[j];
            if (char === '"') {
                inQuotes = !inQuotes;
            } else if (char === ',' && !inQuotes) {
                row.push(currentField);
                currentField = '';
            } else {
                currentField += char;
            }
        }
        row.push(currentField);
        if (!row[0] || !row[0].includes('/')) continue;
        result.push({
            date: row[0].trim(),
            dayOfWeek: row[1].trim(),
            scheduledTime: row[2].trim(),
            brandAmbassador: row[6].trim(),
            storeName: row[7].trim(),
        });
    }
    return result;
}

function parseCsvJuly(content) {
    const lines = content.split('\n');
    const result = [];
    for (let i = 10; i < lines.length; i++) {
        const line = lines[i].trim();
        if (!line) continue;
        const row = [];
        let inQuotes = false;
        let currentField = '';
        for (let j = 0; j < line.length; j++) {
            const char = line[j];
            if (char === '"') {
                inQuotes = !inQuotes;
            } else if (char === ',' && !inQuotes) {
                row.push(currentField);
                currentField = '';
            } else {
                currentField += char;
            }
        }
        row.push(currentField);
        if (!row[0] || !row[0].includes('/')) continue;
        result.push({
            date: row[0].trim(),
            dayOfWeek: row[1].trim(),
            scheduledTime: row[2].trim(),
            brandAmbassador: row[6].trim(),
            storeName: row[7].trim(),
        });
    }
    return result;
}

function main() {
    const juneCsv = fs.readFileSync("/Users/rahulreddyekkati/.gemini/antigravity/brain/5a34422a-6048-43b3-a1a5-074f343d3336/.system_generated/steps/1906/content.md", "utf-8");
    const julyCsv = fs.readFileSync("/Users/rahulreddyekkati/.gemini/antigravity/brain/5a34422a-6048-43b3-a1a5-074f343d3336/.system_generated/steps/1874/content.md", "utf-8");

    const juneRows = parseCsvJune(juneCsv);
    const julyRows = parseCsvJuly(julyCsv);

    // Group June shifts by store + day to find standard time and worker
    const patterns = {};
    juneRows.forEach(r => {
        const store = r.storeName.replace(/\s+/g, ' ').trim().toLowerCase();
        const day = r.dayOfWeek.trim().toLowerCase();
        const key = `${store} | ${day}`;
        
        if (!patterns[key]) {
            patterns[key] = {
                timeOccurrences: {},
                workerOccurrences: {}
            };
        }
        const p = patterns[key];
        p.timeOccurrences[r.scheduledTime] = (p.timeOccurrences[r.scheduledTime] || 0) + 1;
        
        const worker = normalizeWorkerName(r.brandAmbassador);
        if (worker) {
            p.workerOccurrences[worker] = (p.workerOccurrences[worker] || 0) + 1;
        }
    });

    const comparisons = [];
    julyRows.forEach(r => {
        const storeKey = r.storeName.replace(/\s+/g, ' ').trim().toLowerCase();
        const dayKey = r.dayOfWeek.trim().toLowerCase();
        const key = `${storeKey} | ${dayKey}`;
        
        const pattern = patterns[key];
        let juneTime = "N/A";
        let juneWorker = "N/A";
        
        if (pattern) {
            const sortedTimes = Object.entries(pattern.timeOccurrences).sort((a,b) => b[1] - a[1]);
            if (sortedTimes.length > 0) juneTime = sortedTimes[0][0];
            
            const sortedWorkers = Object.entries(pattern.workerOccurrences).sort((a,b) => b[1] - a[1]);
            if (sortedWorkers.length > 0) juneWorker = sortedWorkers[0][0];
        }

        comparisons.push({
            date: r.date,
            day: r.dayOfWeek,
            store: r.storeName,
            julyTime: r.scheduledTime,
            julyWorker: normalizeWorkerName(r.brandAmbassador) || "Open",
            juneTime,
            juneWorker
        });
    });

    console.log("=== COMPARING JULY SCHEDULE VS JUNE PATTERNS ===");
    console.log(JSON.stringify(comparisons, null, 2));
}

main();
